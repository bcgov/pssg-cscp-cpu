...
export class ProgramSummaryTableComponent implements OnInit {
  ...
  ngOnInit() {
    if (this.pa.operationHours && this.pa.operationHours.length) {
      this.operationsSum = this.pa.operationHours
        .map((h) => this.calculateHourTotal(h))
        .reduce((prev, curr) => (prev += curr));
    }
    if (this.pa.standbyHours && this.pa.standbyHours.length) {
      this.standbySum = this.pa.standbyHours
        .map((h) => this.calculateHourTotal(h))
        .reduce((prev, curr) => (prev += curr));
    }
  }
  calculateHourTotal(h: iHours): number {
    if (h.open && h.closed) {
      const open: moment.Moment = moment()
        .hours(Number(h.open.split(":")[0]))
        .minutes(Number(h.open.split(":")[1]));
      const closed: moment.Moment = moment()
        .hours(Number(h.closed.split(":")[0]))
        .minutes(Number(h.closed.split(":")[1]));
